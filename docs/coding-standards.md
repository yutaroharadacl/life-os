...
**規約が優先。設計書は規約の例外を勝手に作れない。**
設計書に規約と異なる方針が書かれていたら、実装者はその場で従わず次のどちらかを選ぶ。

- 規約どおりに実装し、設計書側を是正する（既定）
- 例外が本当に必要なら、**規約側を改訂してから**実装する（ユーザーの承認が必要）

「この設計書の中でだけ有効な例外」を残さない。レビュー時に規約違反として再指摘され、
どちらが正か分からなくなるため（`docs/ai-feedback.md` 2026-08-06 #2）。
...
`role="tablist"` / `role="tab"` / `role="dialog"` のような WAI-ARIA の複合ウィジェットパターンを使うとき、
APG が求めるキーボード操作（矢印キーでの移動、roving tabindex、`aria-controls` と `tabpanel` の対応など）を
どこまで実装するかは**設計書「3. 画面・UI 仕様 / アクセシビリティ」の記述に従う**。

- 設計書に**キーボード操作の要求が明記されていない場合の既定は簡易実装**
  （クリック／タップ＋適切な `role` と `aria-selected` などの状態属性まで）
- フルパターンが必要なら、設計書側に矢印キー移動などを明記してから実装する

ネイティブ要素（`<button>` `<dialog>` `<select>`）で表現できるものは、
ARIA を手書きせずネイティブ要素を使う。
...
新しい要素を足すとき、**新しいクラスを足すのが既定ではない。** 次の順で判断する。

1. 見た目の要求が「既存の○○と同じ」なら、**既存クラスをそのまま再利用する**
2. ただし、再利用するクラス名が**1つの用途に固有な名前**（`.masterLink` など）なら、
   使い回す前に**役割ベースの汎用名へリネームする**（`.navLink`）。
   固有名のまま複数用途で使い回すと、後から読む人が対応関係を誤解する
3. 既存クラスでは満たせない見た目の差分があるときだけ、新規クラスを追加する

設計書が `*.module.scss` を「変更」対象に挙げていても、この判断の結果
CSS の変更が不要になることはある。その場合は設計書側を実態に合わせて更新する。
...
### レビューで挙動を変えたらテスト観点も同じ差分で直す

レビュー指摘を受けて挙動を追加・変更したときは、**同じコミットの中で**次の3つを揃える。

1. 実装
2. テストコード
3. 設計書「9. テスト観点」のチェックリスト

3 の漏れが繰り返し発生している（`docs/ai-feedback.md` 2026-08-06 #2 / 2026-08-07 #1 / 2026-08-18 #3）。
「実装とテストは直したので設計書は後で」を許さない。
