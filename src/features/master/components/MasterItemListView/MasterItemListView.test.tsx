...
describe('MasterItemListView', () => {
  describe('正常系', () => {
    ...
    it('titleが見出しとして表示される', () => {
      render(<MasterItemListView title="カテゴリ管理" itemLabel="カテゴリ" initialItems={[]} />);
    ...
    it('itemLabelが保管場所のとき追加ボタンを押すとモーダルタイトルが保管場所を追加になる', async () => {
      const user = userEvent.setup();
      render(<MasterItemListView title="保管場所管理" itemLabel="保管場所" initialItems={[]} />);
  ...
  describe('境界値', () => {
    ...
    it('initialItemsを省略したとき登録されているカテゴリはありません。が表示される', () => {
      render(<MasterItemListView title="カテゴリ管理" itemLabel="カテゴリ" />);
